...
The AArch64 implementation reuses the existing Shoup tables to derive signed `2^31/q` companions. Unlike Becker et al.'s lazy signed representation, it canonicalizes every butterfly result into `[0, q)`; this simplifies range invariants at the cost of extra per-butterfly reductions. Variable pointwise products use widening Montgomery reduction instead because both operands vary at runtime.
